DROP PROCEDURE IF EXISTS `sync_stock_requirement_transit`;

DELIMITER //
CREATE PROCEDURE sync_stock_requirement_transit(IN exec_date_in DATETIME)
  BEGIN
    SELECT @report_date_start := max(report_date)
    FROM stock_report_requirement_transit; #查询上次报表日期
    ...
    DELETE FROM stock_report_requirement_transit;
    #查询结果插入零时表
    INSERT INTO `stock_report_requirement_transit`
    (`id`,
     `requirement_no`,
     `legaler`,
     `legaler_name`,
    ...
    UPDATE stock_requirement a
      RIGHT JOIN
      stock_report_requirement_transit b ON a.`requirement_no` = b.`requirement_no` AND a.`sku` = b.`sku`
    SET
