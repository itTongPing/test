...
import com.aukey.report.dao.PurchaseReportDao;
...
@Repository
public class PurchaseReportDaoImpl extends BaseDao implements PurchaseReportDao
{
    ...
    {
        PageResult<PurchaseReportVO> pageResult = new PageResult<PurchaseReportVO>(pageParam.getPageNum(),
		...
		if(infoSize > SPTEP_SIZE){
			...
			while (fromIndex < infoSize) {
				...
				appendInvoiceInfo(rows.subList(fromIndex, toIndex));
				fromIndex = fromIndex + SPTEP_SIZE;
			}
		} else {
			appendInvoiceInfo(rows);
		}        
    ...
    private void appendInvoiceInfo(List<PurchaseReportVO> rows){
    	...
    	if(CommonUtil.isEmpty(purchaseOrderIds.toString())){
         ...
         String invoiceSql ="SELECT ts1.purchase_no, ts1.sku, ts1.stock_number, GROUP_CONCAT(DISTINCT ts2.invoice_no) AS invoiceCode, SUM(IFNULL(invoice_num, 0)) AS invoice_num FROM ( " +
         					"	SELECT t.purchase_order_id AS purchase_no, t.sku, GROUP_CONCAT(DISTINCT t1.warehouse_number) AS stock_number "+
         					"	FROM invoice.purchase_invoice t " +
         					"	LEFT JOIN invoice.invoice t1 ON t1.purchase_invoice_id = t.id " +
         					"	WHERE t.purchase_order_id IN ("+purchaseOrderIds.toString().substring(1) + ") " +
         					"	GROUP BY t.id " +
         					"	)ts1 " + 
         					"	LEFT JOIN invoice.purchase_sku_invoice_record ts2 ON ts2.del_type = 'N' AND  ts1.purchase_no = ts2.purchase_order_id AND ts1.sku = ts2.sku " +
         					"	LEFT JOIN invoice.invoice_tax_collect ts3 ON ts2.record_id = ts3.invoice_record_id AND ts2.invoice_no = ts3.invoice_code " +
         					"	WHERE ts3.review_status_id = 'APPROVED' " +
         					"	GROUP BY ts1.purchase_no, ts2.sku " +
         					"	ORDER BY ts1.sku";
         final BeanPropertyRowMapper<PurchaseReportVO> rowMapper = new BeanPropertyRowMapper<PurchaseReportVO>(PurchaseReportVO.class);
         List<PurchaseReportVO> invoiceInfos = getJdbcTemplate().query(invoiceSql, rowMapper);
         
         String warehosueSql = "SELECT t.purchase_order_id AS purchase_no, t.sku,  t1.warehouse_number AS stock_number, t1.warehouse_goods_num AS stock_count " +
        		 			   "FROM invoice.purchase_invoice t " +
        		 			   "LEFT JOIN invoice.invoice t1 ON t1.purchase_invoice_id = t.id " +
        		 			   "WHERE t.purchase_order_id IN ("+purchaseOrderIds.toString().substring(1) + ") " +
        		 			   "GROUP BY t.id, t1.warehouse_number " +
        		 			   "ORDER BY t1.warehouse_date ";
         List<PurchaseReportVO> warehouseInfos = getJdbcTemplate().query(warehosueSql, rowMapper);
         
         for (PurchaseReportVO report : warehouseInfos) {
        	 String purchaseOrderId = report.getPurchase_no();
        	 String sku = report.getSku();
        	 String warehouseOrderId = report.getStock_number();
        	 report.setBill_status("未开票");
			 report.setNo_bill_count(report.getStock_count());
			 report.setInvoiceCode(null);
        	 for (PurchaseReportVO invoiceInfo : invoiceInfos) {
        		 String warehouseOrderIds = invoiceInfo.getStock_number();
        		 int invoice_num = invoiceInfo.getInvoice_num();
        		 if (purchaseOrderId.equals(invoiceInfo.getPurchase_no())
        				 && sku.equals(invoiceInfo.getSku())
        				 && warehouseOrderIds.contains(warehouseOrderId)) {
        			 if(invoice_num > report.getStock_count()){
        				 invoiceInfo.setInvoice_num(invoice_num - report.getStock_count());
        				 report.setNo_bill_count(0);
        				 report.setInvoiceCode(invoiceInfo.getInvoiceCode());
        				 report.setBill_status("全部开票");
        			 } else if(invoice_num == report.getStock_count()){
        				 invoiceInfo.setInvoice_num(0);
        				 report.setNo_bill_count(0);
        				 report.setInvoiceCode(invoiceInfo.getInvoiceCode());
        				 report.setBill_status("全部开票");
        			 } else {
        				 invoiceInfo.setInvoice_num(0);
        				 report.setNo_bill_count(report.getStock_count() - invoice_num);
        				 if(invoice_num == 0){
        					 report.setBill_status("未开票");							
        				 } else {
        					 report.setBill_status("部分开票");														
        					 report.setInvoiceCode(invoiceInfo.getInvoiceCode());
        				 }
        			 }
        			 break;
         ...
         for (PurchaseReportVO row : rows) {
        	 ...
        	 String warehouseOrderId = row.getStock_number();
        	 
        	 row.setInvoice_num(0);
        	 row.setNo_bill_count(row.getStock_count());
        	 row.setInvoiceCode(null);
        	 row.setBill_status("未开票");
        	 row.setNoMakeInvoice(row.getNoMakeInvoice());
        	 row.setMakeInvoice(row.getMakeInvoice());
        	 
			for (PurchaseReportVO warehouseInfo : warehouseInfos) {
				if (purchaseOrderId.equals(warehouseInfo.getPurchase_no())
       				 && sku.equals(warehouseInfo.getSku())
       				 && warehouseOrderId.equals(warehouseInfo.getStock_number())) {
					row.setInvoice_num(warehouseInfo.getInvoice_num());
					row.setNo_bill_count(warehouseInfo.getNo_bill_count());
					row.setInvoiceCode(warehouseInfo.getInvoiceCode());
					row.setBill_status(warehouseInfo.getBill_status());
					row.setNoMakeInvoice(row.getNoMakeInvoice());
					row.setMakeInvoice(row.getMakeInvoice());
					break;
				}
			...
			for (PurchaseReportVO info : purchaseTaxRateInfos) {
				if(purchaseOrderId.equals(info.getPurchase_no())){
