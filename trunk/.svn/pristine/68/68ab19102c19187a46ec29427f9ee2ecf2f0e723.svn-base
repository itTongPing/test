...
function getData(){
    ...
    //sku名称
    if($("#skuName").val()){
        $("#skuName").val($("#skuName").val().trim().replace(/\s+/g,',').replace(/,{2,}/g,','))
    }
    var Sku_name = $("#skuName").val().split(',');
    //运输方式
    ...
    //含税
    var TaxIncluded = $("#TaxIncluded").val();
    //需求部门
    var Department = $("#Department").val();
    var exportData = {
            sku:Sku,
            sku_name:Sku_name,
            transport_type:salesType,
            inventory_name:warehouse,
            legaler:LegalPerson,
            country:Country,
            include_tax:TaxIncluded,
...
$(function(){
    //法人主体下拉列表参数处理
    $("#LegalPerson").prev('.dropdown-menu.open').find('.text').each(function(){
        var newStringArr = $(this).text().split("=")
        $(this).text(newStringArr[0]);
        $(this).val(newStringArr[1]);
    })
    $("#LegalPerson option").not(':first').each(function(){
        var newStringArr = $(this).text().split("=")
        $(this).text(newStringArr[0]);
        $(this).val(newStringArr[1]);
    })
...
$(function(){
    $("#queryForm").on('submit',function(){
        ...
        $.ajax({
            url:'/report/inventory/preExport',
            contentType:'json',
            async:false,
            method:'get',
            data:Data,
