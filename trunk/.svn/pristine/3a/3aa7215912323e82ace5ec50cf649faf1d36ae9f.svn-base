...
var TableInit = function()
{
	...
	{
		var firstTransferColumns = [];
		//-----列名前部分
		firstTransferColumns.push(
				{
                    field : 'transferNo',
                    title : '调拨单号',
                    align : 'center',
		...
		bootstrapTable = $('#firstTransferTable').bootstrapTable(
	            {
	                url:'/report/centerStock/search/firstAirTransfer', // 请求后台的URL（*）
	                method: 'get', // 请求方式（*）
	                ...
	                columns : firstTransferColumns,
	                    onLoadSuccess: function (data) {
	                    	flyer.scrollBar($('#firstTransferTable')); //初始化滚动条
	...
	oTableInit.queryParams = function (params)
	{
		...
		var type = $("#type").val().trim().replace(/^\s+|\s+$/g, ""); // type;
		params.sku = sku;
		params.type = type;
...
{
	 keyNumbers = [];
	$("#firstTransferTable").bootstrapTable('destroy');
	var oTable = new TableInit();
    oTable.Init();
}
//重置搜索条件
