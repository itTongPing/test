...
var TableInit = function()
{
	...
	{
		var transferWarehouseColumns = [];
		//-----列名前部分
		transferWarehouseColumns.push( {
            field : 'warehouseName',
            title : '仓库',
            align : 'center',
            width : 100
		...
		//----列名尾部
		bootstrapTable = $('#transferWarehouseTable').bootstrapTable(
	            {
	                url:'/report/centerStock/search/transferWarehouseCount', // 请求后台的URL（*）
	                method: 'get', // 请求方式（*）
	                ...
	                columns : transferWarehouseColumns,
	                    onLoadSuccess: function (data) {
	                    	flyer.scrollBar($('#transferWarehouseTable')); //初始化滚动条
...
{
	 keyNumbers = [];
	$("#transferWarehouseTable").bootstrapTable('destroy');
	var oTable = new TableInit();
    oTable.Init();
}
//重置搜索条件
