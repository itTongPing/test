...
//		}
		...
		var TableInit = function() {
		    ...
		    oTableInit.init = function() {
		        $('#table').bootstrapTable({
		            url: '/storageReport/search', // 请求后台的URL（*）
                    method: 'get', // 请求方式（*）
                    ...
                    width: 'auto',
                    uniqueId: "qualityControlId", // 每一行的唯一标识，一般为主键列
                    cardView: false,
		            ...
		            columns: [{
		                ...
		                {
		                    field: 'nondefectiveNumber',
		                    title: '良品入库数量',
		                    align: 'center',
		                    formatter: 'pcsFormatter',
		                }, 
		                {
		                    field: 'rejectsNumber',
		                    title: '不良品入库数量',
		    ...
		    oTableInit.queryParams = function(params) {
		        ...
		        if (searchFlag) {
		           ...
		           // var department=$("#department").val().join(',')||'';//需求部门
		            ...
		            var saleName = $("#saleName").val().trim()||'';
		            
		            //目标仓库
		            var keys = [];
		            $(".toGoodsInquiry  .keywords>span").each(function(){
		            	keys.push($(this).data('key'));
		            });
		            
		            params.stockId =  keys.join(',')|| '';
		...
		$("#table").on("post-body.bs.table",function(){
			$("td.skuNameWidth").each(function(i,t){
				...
				$(this).attr("title",$("#table").find(a).text());
			});
		})
		...
		//导出数据
		 function exportStorageList() {
				...
				if(keyNumbers.length > 0 ){//如果存在 勾选
		            ...
		            +"&legaler="+legaler+"&isTax="+isTax+"&transferType="+transferType+"&buyer="+buyer+"&supplier="+supplier;

		            
//		            $.ajax({
//				        method: "GET",
//				        url: "/storageReport/getCount?"+queryString,
//				        contentType: "application/json; charset=utf-8",
//				        beforeSend: function(request) {
//				            if (isAllNotEmpty(header, token))
//				                request.setRequestHeader(header, token);
...
//		     }
     ...
     { console.log(stock+"---"+_ctx)
         var wa = $(".warehouse");
         $.ajax({
             url :"/storageReport/getSupplierInfo",
             type : 'get',
             async : true,
             data : {
                 supplyAlias : stock
...
/****************************************************发货计划列表分页选中功能  开始******************************************************/
		 ...
		 $table.on('check.bs.table check-all.bs.table',function()
		 {
		     ...
		     {
		         if ($.inArray(obj.storageNumber,keyNumbers) == -1)
		         {
		             selections.push(obj);
		 ...
		 $table.on('uncheck.bs.table',function(e, row)
		 {
		     ...
		     keyNumbers.splice($.inArray(row.storageNumber,keyNumbers),1);
		     console.info("------");
		     console.info(selections);
		     console.info(keyNumbers);
		 });
...
//精确金额
 function formatterMoney(value,row){
 	var r = parseFloat(value).toFixed(2);
 	return r;
 }
