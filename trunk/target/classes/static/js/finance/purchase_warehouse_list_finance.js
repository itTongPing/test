...
function initTable()
{
	$('#table01').bootstrapTable({
	    url: '/report/finance/search', // 请求后台的URL（*）
	    method: 'get', // 请求方式（*）
	    ...
	    onLoadSuccess : function(data)
	    {
	        ...
	        $.ajax({
	            url : "/report/finance/searchCount",
	            type : 'get',
	            async : false,
	            data : getData(),
...
function exportExcel(){
	...
	$.ajax({
		url:'/report/finance/preExport'+paramsL,
		contentType:'json',
		async:false,
		method:'get',
		data:Data,
		success:function(req){
			if(req.success){
				$.jGrowl('文件通过邮件发送请耐心等待');
				allowExport = false;
				//请求邮件导出
				$.ajax({
					url:'/report/finance/emailExport'+paramsL,
					contentType:'json',
					async:true,
					method:'get',
					data:Data,
	...
	if(allowExport){
		window.open('/report/finance/export'+paramsL);
	}
}
/****导出结束*****/
 ...
 * 检查采购时间
 */
 validPurchaseDate = function(){
	var fromPurchaseDate = $('#from_purchase_date').val();
	var toPurchaseDate =  $('#to_purchase_date').val();
	return compareDate(fromPurchaseDate,toPurchaseDate,"采购");
}
 ...
 */ 
 validToDate = function (){
	...
	return compareDate(fromDate,toDate,"入库");
}
...
var searchResult = function(){
	 ...
	 * 判断时间
	 */
	if(!validPurchaseDate() || !validToDate()){
		return;
	}
	...
	/*//ajax请求
	$.ajax({
        url : _ctx + "report/finance/searchCount",
        type : 'get',
        async : false,
        data : getData(),
        success : function(res)
...
$(function(){
	...
	$("#table01").on("load-success.bs.table",function(){
		...
		//采购本币金额
		var purchaseCurrencySum = "purchaseCurrencySum";
		var allPurchaseCurrencySum=calculationData(getData,purchaseCurrencySum);
		//获取入库数量
...
//		$("#table01 tbody").append('<tr><td>总数</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>'+allPurchaseCount+'</td><td>-</td><td>-</td><td>'+allPurchaseSum+'</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>'+allstock_count+'</td><td>'+allstock_sum+'</td><td>-</td><td>-</td><td>-</td><td>'+allno_bill_count+'</td><td>'+allnoMakeInvoice.toFixed(2)+'</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td><td>-</td></tr>')
		...
		$('.purchaseCurrencySum').eq(0).find(".th-inner").text('采购本币金额(' + allPurchaseCurrencySum.toFixed(2) + ')' );
...
function getCount(res)
{
	...
	var sum = parseFloat(data.moneySum).toFixed(2);
	if(isNaN(sum))
	{
		sum = 0.00;
	}
	$("#recordCount").remove();
	var $node = $("<div id='recordCount'>入库总数量:"+count+"				入库总金额:"+sum+"</div>");
	$('#table01').after($node);
}
