...
DELIMITER //
CREATE PROCEDURE sync_stock_transit(IN exec_date_in DATETIME)
  BEGIN
    SELECT @report_date_start := max(report_date)
    FROM stock_report_record_transit; #查询上次报表日期
    ...
    DELETE FROM stock_report_record_transit;
    #查询结果插入零时表
    INSERT INTO `stock_report_record_transit`
    (`id`,
     `legaler`,
     `legaler_name`,
     `department`,
    ...
    UPDATE stock_report a
      LEFT JOIN
      stock_report_record_transit b ON a.`legaler` = b.`legaler`
